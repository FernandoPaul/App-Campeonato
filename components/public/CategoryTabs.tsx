...
function MatchCard({ match }: { match: Match }) {
    ...
    return (
        <div
            className={`relative bg-zinc-900/80 border rounded-xl overflow-hidden transition-all hover:border-zinc-600 ${isLive ? "border-red-500/40" : "border-zinc-800/60"
                }`}
        >
            {isLive && (
            ...
            <div className="px-4 py-3 flex items-center gap-3">
                {/* Home */}
                <div
                    className={`flex items-center gap-2.5 flex-1 min-w-0 ${awayWin ? "opacity-50" : ""
                        }`}
                >
                    <TeamAvatar team={match.homeTeam} />
                    <span
                        className={`text-sm font-semibold truncate ${homeWin ? "text-white" : "text-zinc-300"
                            }`}
                    >
                        {match.homeTeam.name}
                    </span>
                ...
                <div className="flex flex-col items-center flex-shrink-0 min-w-[72px]">
                    {isFinished || isLive ? (
                        <div className="flex items-center gap-2">
                            <span
                                className={`text-xl font-black tabular-nums ${homeWin ? "text-white" : "text-zinc-300"
                                    }`}
                            >
                                {match.homeScore ?? 0}
                            </span>
                            ...
                            <span
                                className={`text-xl font-black tabular-nums ${awayWin ? "text-white" : "text-zinc-300"
                                    }`}
                            >
                                {match.awayScore ?? 0}
                            </span>
                ...
                {/* Away */}
                <div
                    className={`flex items-center gap-2.5 flex-1 min-w-0 justify-end ${homeWin ? "opacity-50" : ""
                        }`}
                >
                    <span
                        className={`text-sm font-semibold truncate text-right ${awayWin ? "text-white" : "text-zinc-300"
                            }`}
                    >
                        {match.awayTeam.name}
                    </span>
...
export default function CategoryTabs({
    ...
    const nextRound =
        currentRoundIdx < rounds.length - 1 ? rounds[currentRoundIdx + 1] : null
                    ...
                    <table className="min-w-full">
                        <thead>
                            <tr className="bg-zinc-950/60 border-b border-zinc-800/60">
                                <th className="pl-4 pr-2 py-3 text-left text-[11px] font-bold text-zinc-500 uppercase tracking-widest w-8">
                                    #
                                </th>
                                <th className="px-2 py-3 text-left text-[11px] font-bold text-zinc-500 uppercase tracking-widest">
                                    Equipo
                                </th>
                                <th className="px-3 py-3 text-center text-[11px] font-bold text-zinc-500 uppercase tracking-widest">
                                    PJ
                                </th>
                                <th className="px-3 py-3 text-center text-[11px] font-bold text-zinc-500 uppercase tracking-widest hidden sm:table-cell">
                                    G
                                </th>
                                <th className="px-3 py-3 text-center text-[11px] font-bold text-zinc-500 uppercase tracking-widest hidden sm:table-cell">
                                    E
                                ...
                                <th className="px-3 py-3 text-center text-[11px] font-bold text-zinc-500 uppercase tracking-widest hidden md:table-cell">
                                    GF
                                </th>
                                <th className="px-3 py-3 text-center text-[11px] font-bold text-zinc-500 uppercase tracking-widest hidden md:table-cell">
                                    GC
                                ...
                                <th className="px-4 py-3 text-center text-[11px] font-bold text-blue-400 uppercase tracking-widest">
                                    Pts
                                </th>
                            </tr>
                        </thead>
                                    ...
                                    >
                                        ...
                                        <td className="px-2 py-3.5">
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <span
                                                    className={`text-sm font-semibold ${isTop3 ? "text-white" : "text-zinc-300"
                                                        }`}
                                                >
                                                    {s.team.name}
                                                </span>
                                        ...
                                        <td className="px-3 py-3.5 text-center text-sm text-zinc-300 font-medium tabular-nums">
                                            {s.played}
                                        </td>
                                        <td className="px-3 py-3.5 text-center text-sm text-zinc-400 tabular-nums hidden sm:table-cell">
                                            {s.won}
                                        </td>
                                        <td className="px-3 py-3.5 text-center text-sm text-zinc-400 tabular-nums hidden sm:table-cell">
                                            {s.drawn}
                                        ...
                                        <td className="px-3 py-3.5 text-center text-sm text-zinc-400 tabular-nums hidden md:table-cell">
                                            {s.goalsFor}
                                        </td>
                                        <td className="px-3 py-3.5 text-center text-sm text-zinc-400 tabular-nums hidden md:table-cell">
                                            {s.goalsAgainst}
        ...
        <div className="space-y-4">
            ...
            {rounds.length > 0 && (
                <div className="flex items-center gap-2">
                    ...
                    {prevRound ? (
                        <Link
                            href={`/${categorySlug}?tab=calendario&jornada=${encodeURIComponent(prevRound)}`}
                    ...
                    {nextRound ? (
                        <Link
                            href={`/${categorySlug}?tab=calendario&jornada=${encodeURIComponent(nextRound)}`}
                            className="w-8 h-8 flex items-center justify-center rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white hover:border-zinc-600 transition-all flex-shrink-0"
                        >
